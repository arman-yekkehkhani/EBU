...
class MyImageToPytorch(gym.ObservationWrapper):
    def __init__(self, env):
        ...
        self.observation_space = gym.spaces.Box(low=np.moveaxis(old_low, 2, 0), high=np.moveaxis(old_high, 2, 0),
                                                shape=(old_shape[-1], old_shape[0], old_shape[1]), dtype=old_type)
